...
export const generateSocialPost = async (recipe: Recipe): Promise<ReelScript> => {
  const ai = createAI();
  const prompt = `Atue como Especialista em Marketing para Instagram e Facebook. Crie uma postagem viral com foto para a receita: "${recipe.title}".
  - Hook: Uma frase inicial que chame atenção imediata.
  - Body: Corpo do texto vendendo a ideia da receita de forma irresistível.
  - CTA: Chamada para ação pedindo para comentar ou clicar no link.
  - Hashtags: 5 a 8 hashtags estratégicas juntas (ex: #ReceitaFacil #Bolo).
  Ignore 'visualPrompt' (preencha com "none").`;
