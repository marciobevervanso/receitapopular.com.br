...
export const SearchBar: React.FC<SearchBarProps> = ({ recipes, onSelectRecipe, language = 'pt' as Language }) => {
  ...
  useEffect(() => {
    ...
    const filtered = recipes.filter(recipe => {
      ...
      // 2. Advanced Filters
      // Logic: For each category, if filters are selected, recipe tags must contain AT LEAST ONE of them.
      // AND logic is applied between categories.
      const matchesFilters = Object.entries(selectedFilters).every(([category, selectedOptions]) => {
        ...
        // Check if recipe tags overlap with selected options for this category
        // We assume recipe.tags contains strings like "Vegano", "Italiana", etc.
  ...
  return (
    <div ref={wrapperRef} className="relative max-w-4xl mx-auto px-4 -mt-8 mb-12 z-20">
      ...
      <div className="relative shadow-xl shadow-gray-200/50 rounded-2xl bg-white transition-all duration-300 focus-within:shadow-2xl z-30">
        <div className="flex items-center px-4 md:px-6 py-4 gap-4">
          ...
          <div className="flex-grow flex items-center">
            ...
            <input
              type="text"
              className="w-full bg-transparent border-none outline-none text-pop-dark placeholder-gray-400 font-medium text-lg h-full"
                ...
                {results.map((recipe) => (
                    ...
                    >
                        ...
                        <div className="flex-1">
                            <h4 className="font-serif font-bold text-pop-dark group-hover:text-pop-red transition-colors text-lg">{recipe.title}</h4>
