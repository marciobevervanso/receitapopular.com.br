...
export const CategoryPage: React.FC<CategoryPageProps> = ({ 
  ...
  const categoryAliases: Record<string, string[]> = {
    'Sobremesas': ['doce', 'bolo', 'torta', 'chocolate', 'sorvete', 'sobremesa', 'brigadeiro', 'pudim', 'pavê'],
    'Café da Manhã': ['café', 'manha', 'pão', 'omelete', 'panqueca', 'tapioca', 'vitamina', 'bolo'],
    'Almoço': ['almoço', 'carne', 'frango', 'arroz', 'feijão', 'principal', 'peixe', 'porco'],
    'Jantar': ['jantar', 'sopa', 'leve', 'rápido', 'noite', 'creme', 'caldo', 'pizza'],
    'Fitness': ['fit', 'fitness', 'low carb', 'saudável', 'whey', 'proteína', 'treino'],
    'Airfryer': ['airfryer', 'fritadeira', 'sem óleo', 'assado', 'crocante'],
    'Saudável': ['fit', 'saudável', 'salada', 'legumes', 'natural', 'light', 'detox', 'vegano', 'vegetariano'],
    'Massas': ['massa', 'macarrão', 'lasanha', 'nhoque', 'espaguete', 'pizza', 'rondelli'],
    'Drinks': ['drink', 'bebida', 'suco', 'coquetel', 'batida', 'smoothie', 'chá'],
    'Pães': ['pão', 'massa', 'fermento', 'padaria', 'brioche', 'focaccia', 'baguete'],
  };

  const filteredRecipes = recipes.filter(r => {
    const aliases = categoryAliases[categoryName] || [];
    const searchTerms = [categoryName.toLowerCase(), ...aliases.map(a => a.toLowerCase())];
    
    // Check tags
    const matchTag = r.tags.some(tag => {
      const t = tag.toLowerCase();
      return searchTerms.some(term => t.includes(term) || term.includes(t));
    });

    // Check title (fallback)
    const matchTitle = searchTerms.some(term => r.title.toLowerCase().includes(term));
