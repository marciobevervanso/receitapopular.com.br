       ...
       <div className="bg-white p-8 rounded-3xl border border-gray-100 shadow-sm space-y-8">
          ...
          <div>
             ...
             <div className="space-y-4">
                ...
                <div>
                   <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Google Analytics (GA4)</label>
                   <input 
                     type="text" 
                     placeholder="G-XXXXXXXXXX"
                     value={formData.googleAnalyticsId || ''}
                     onChange={e => handleChange('googleAnalyticsId', e.target.value)}
                     className="w-full px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl font-bold font-mono placeholder:font-sans text-sm"
                   />
                   <p className="text-[10px] text-gray-400 mt-1">O código de rastreamento será injetado automaticamente no cabeçalho inteiro do site.</p>
                </div>
                <div>
                   <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Meta Pixel (Facebook)</label>
                   <input 
                     type="text" 
          ...
          {/* Integrations (Coolify & n8n) */}
          <div>
             <h3 className="font-bold text-pop-dark uppercase text-sm mb-4 border-b border-gray-100 pb-2">Integrações (Serviços Externos)</h3>
             <div className="space-y-4">
                
                {/* Image Converter (Coolify VPS) */}
                <div>
                   <label className="block text-xs font-bold text-purple-600 uppercase tracking-widest mb-2">
                      Conversor de Imagens (VPS Própria)
                   </label>
                   <div className="flex gap-2">
                      <span className="px-3 py-3 bg-purple-50 border border-purple-100 rounded-xl text-purple-600 font-bold text-sm">POST</span>
                      <input 
                        type="text" 
                        value={formData.customConverterUrl || ''}
                        onChange={e => handleChange('customConverterUrl', e.target.value)}
                        className="flex-1 px-4 py-3 bg-purple-50 border border-purple-100 rounded-xl text-sm font-mono text-purple-700"
                        placeholder="https://imgconvert.seusite.com.br/convert?quality=82"
                      />
                   </div>
                   <p className="text-[10px] text-gray-400 mt-2">
                      Substitui o n8n. O sistema enviará a imagem diretamente para este endpoint.
                   </p>
                </div>

                {/* Legacy n8n Image Optimization (Fallback) */}
                <div>
                   <label className="block text-xs font-bold text-gray-400 uppercase tracking-widest mb-2">Webhook n8n (Otimização de Imagens)</label>
                   <div className="flex gap-2">
                      <span className="px-3 py-3 bg-gray-100 border border-gray-200 rounded-xl text-gray-500 font-bold text-sm">POST</span>
                      <input 
                        type="text" 
                        value={formData.n8nImageOptimizationUrl || ''}
                        onChange={e => handleChange('n8nImageOptimizationUrl', e.target.value)}
                        className="flex-1 px-4 py-3 bg-gray-50 border border-gray-200 rounded-xl text-sm font-mono text-gray-600"
                        placeholder="https://seu-n8n.com/webhook/optimize..."
                      />
                   </div>
                   <p className="text-[10px] text-gray-400 mt-2">
                      Use este campo para conectar seu workflow de otimização de imagens no n8n.
                   </p>
                </div>
